...
_REVIEW_SYSTEM = (
    ...
    "CRITICAL — THE WORLD CHANGES BETWEEN TURNS. Each turn is stamped with its date and time. "
    "Watches get created, deleted, started and stopped between messages, and the app is updated. "
    "So a turn saying 'you have four watches' and a LATER turn saying 'you have one watch' is "
    "almost certainly correct both times — three were deleted in between. The same goes for "
    "changed version numbers, changed counts, and changed watch states. Only call something a "
    "contradiction when the two turns are CLOSE TOGETHER IN TIME and nothing in between could "
    "explain the change. When in doubt, say nothing — a false contradiction sends someone hunting "
